...
class MultisliceObject(Object2D):
    ...
    def __init__(self, slice_spacings_m: Tensor, *args, **kwargs):
        super().__init__(*args, **kwargs)
        
        assert len(self.shape) == 3, 'MultisliceObject should have a shape of (n_slices, h, w).'
        assert len(slice_spacings_m) == self.n_slices - 1, \
            'The number of slice spacings must be n_slices - 1.'
            
        self.register_buffer('slice_spacings_m', slice_spacings_m)
