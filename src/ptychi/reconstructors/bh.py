...
class BHReconstructor(AnalyticalIterativePtychographyReconstructor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.options.method == 'CG':
            self.forward_model.record_intermediate_variable('eta_o', 0)
            self.forward_model.record_intermediate_variable('eta_op', 0)
            self.forward_model.record_intermediate_variable('eta_p', 0)
            self.forward_model.record_intermediate_variable('eta_pos', 0)
    ...
    def compute_updates(self, indices: torch.Tensor, y_true: torch.Tensor) -> tuple[torch.Tensor, ...]:
        """
        ...
        if o_opt and p_opt and pos_opt:
            ...
            (delta_o, delta_p) = self.compute_updates_object_probe(
                op, p, psi_far, gradF, d)
            delta_pos = None
    ...
    def compute_updates_object(self, op, p, psi_far, gradF, d):
        ...
        if self.current_epoch == 0 or self.options.method == 'GD':
            ...
            eta_op = self.forward_model.intermediate_variables["eta_op"]
        ...
        if self.options.method == 'CG':
            ...
            self.forward_model.record_intermediate_variable('eta_op', eta_op)
    ...
    def compute_updates_object_probe(self, op, p, psi_far, gradF, d):
        ...
        grad_o, grad_op = self.gradient_o(p, gradF)
        grad_p = self.parameter_group.probe.options.rho * \
            self.gradient_p(op, gradF)
    ...
    def compute_updates_object_probe_positions(self, op, p, pos, psi_far, gradF, d):
        ...
        grad_p = self.parameter_group.probe.options.rho * \
            self.gradient_p(op, gradF)
        grad_pos = self.parameter_group.probe_positions.options.rho * \
            self.gradient_pos(op, p, pos, gradF)
        ...
        if self.current_epoch == 0 or self.options.method == 'GD':
            ...
            eta_op = self.forward_model.intermediate_variables["eta_op"]
            eta_p = self.forward_model.intermediate_variables["eta_p"]
            eta_pos = self.forward_model.intermediate_variables["eta_pos"]
        ...
        if self.options.method == 'CG':
            ...
            self.forward_model.record_intermediate_variable('eta_op', eta_op)
            self.forward_model.record_intermediate_variable('eta_p', eta_p)
            self.forward_model.record_intermediate_variable('eta_pos', eta_pos)
    ...
    def gradientF(self, psi_far, d):
        ...
        res = 2 * \
            self.forward_model.free_space_propagator.propagate_backward(td)
        return res
    ...
    def hessianF(self, psi_far, psi_far1, psi_far2, data):
        ...
        v2 = torch.sum(d0 * self.reprod(l0, psi_far1) *
                       self.reprod(l0, psi_far2))
        return 2 * (v1 + v2)
    ...
    def gradient_o(self, p, gradF):
        ...
        patches = obj.extract_patches_function(o, self.positions + obj.center_pixel,
    ...
    def calc_alpha_object_probe(self, p, do1, dp1, do2, dp2, d, gradF, psi_far, op, dop2):
        ...
        bottom = self.redot(gradF, d2m2) + \
            self.hessianF(psi_far, Ldm2, Ldm2, d)
        return top / bottom

    def calc_alpha_object_probe_positions(self, p, do1, dp1, dpos1, do2, dp2, dpos2, d, gradF, psi_far, op, dop2):
        '''Step length for the object, probe and positions update'''

        top = -self.redot(do1, do2) - self.redot(dp1, dp2) - \
            torch.sum(dpos1 * dpos2)
        ...
        w2 = xi1**2 * dpos2[:, 0]**2 + 2 * xi1 * xi2 * \
            (dpos2[:, 0] * dpos2[:, 1]) + xi2**2 * dpos2[:, 1]**2
    ...
    def calc_beta_object_probe(self, p, dp1, dp2, d, gradF, psi_far, op, dop1, dop2):
        ...
        top = self.redot(gradF, d2m1) + \
            self.hessianF(psi_far, Ldm1, Ldm2, d)
        bottom = self.redot(gradF, d2m2) + \
            self.hessianF(psi_far, Ldm2, Ldm2, d)
        return top / bottom

    def calc_beta_object_probe_positions(self, p, dp1, dp2, dpos1, dpos2, d, gradF, psi_far, op, dop1, dop2):
        ...
        d2m1 = p * (dt12 + dt21 + d2t1) + dp1 * \
            (dop2 + dt2) + dp2 * (dop1 + dt1)
